...
const search    = ref("");
const lecturers = ref([]);
const form      = ref({ code: "", name: "", program_id: "", year_level: 1, lecturer_id: "" });
...
function openCreate() {
  editing.value   = null;
  form.value      = { code: "", name: "", program_id: "", year_level: 1, lecturer_id: "" };
  showModal.value = true;
}
function openEdit(u) {
  editing.value   = u;
  form.value      = { code: u.code, name: u.name, program_id: u.program_id, year_level: u.year_level, lecturer_id: u.lecturer_id || "" };
  showModal.value = true;
}
async function save() {
  saving.value = true;
...
<template>
  <BasePageHeading title="Units" subtitle="Course units and their assigned lecturers">
    <template #extra>
      <div class="d-flex gap-2">
        <input v-model="search" type="search" class="form-control form-control-sm" placeholder="Search…" style="width:180px;"/>
        <button v-if="authStore.userCan('units:write')" class="btn btn-primary btn-sm" @click="openCreate">
      ...
      <div class="table-responsive">
        <table class="table table-hover mb-0">
          <thead class="bg-body-light">
            <tr>
              <th>Code</th><th>Name</th><th>Programme</th><th>Yr</th>
              <th v-if="authStore.userCan('units:read')">Lecturer</th>
              <th v-if="authStore.userCan('units:write')" class="text-end">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="u in filtered" :key="u.id">
              <td><span class="badge" style="background:#331B11;font-size:11px;">{{ u.code }}</span></td>
              <td class="fw-semibold">{{ u.name }}</td>
              <td class="text-muted">{{ progName(u.program_id) }}</td>
              <td>{{ u.year_level || "—" }}</td>
              <td v-if="authStore.userCan('units:read')">{{ lecName(u.lecturer_id) }}</td>
              <td v-if="authStore.userCan('units:write')" class="text-end">
                <button class="btn btn-sm btn-alt-secondary" @click="openEdit(u)">
  ...
  <BaseModal :show-modal="showModal" :title="editing ? 'Edit Unit' : 'New Unit'" @close="showModal=false">
    <div class="row g-3">
      <div class="col-md-4">
        <label class="form-label fw-medium">Code *</label>
        <input v-model="form.code" type="text" class="form-control" placeholder="CS301"/>
      </div>
      <div class="col-md-8">
      ...
      <div class="col-md-6">
        ...
        <input v-model.number="form.year_level" type="number" class="form-control" min="1" max="6"/>
      </div>
      <div v-if="authStore.userCan('users:read')" class="col-12">
        ...
        <select v-model="form.lecturer_id" class="form-select">
          ...
          <option v-for="l in lecturers" :key="l.id" :value="l.id">{{ l.full_name || l.username }}</option>
        </select>
      </div>
    </div>
    <template #footer>
