...
// Schema Aligned: unit_id, cohort_id, hall_id, lecturer_id, day_of_week, start_time, end_time
const form = ref({
  unit_id: "",
  cohort_id: "",
  hall_id: "",
...
onMounted(async () => {
  ...
  // Fetch users with LECTURER role for the dropdown
  const { data } = await api.get("/users").catch(() => ({ data: [] }));
  const allUsers = data.items ?? data;
  lecturers.value = allUsers.filter((u) =>
    u.roles?.some((r) => r.name === "LECTURER")
  );
});

// Admin sees everything, but can filter by Class (Cohort)
const filtered = computed(() =>
  filterCls.value
    ? store.timetable.filter((e) => e.cohort_id == filterCls.value)
    : store.timetable
);

const getCell = (d, s) =>
  filtered.value.filter((e) => e.day_of_week === d && e.start_time === s);
const unitName = (id) =>
  store.units.find((u) => u.id === id)?.name || `Unit #${id}`;
const hallName = (id) =>
  store.halls.find((h) => h.id === id)?.name || (id ? `Hall #${id}` : "—");
const lecName = (id) =>
  lecturers.value.find((l) => l.id === id)?.full_name || "TBA";
...
async function saveEntry() {
  saving.value = true;
  try {
    // Map form to backend schema requirements
    const payload = {
      ...
      lecturer_id: form.value.lecturer_id
        ? parseInt(form.value.lecturer_id)
        : null,
...
<template>
  <BasePageHeading
    ...
    subtitle="System-wide lecture schedule"
  >
    <template #extra>
        ...
        <table class="table table-bordered mb-0">
          <thead class="bg-body-light text-center">
            <tr>
              ...
              <th v-for="d in DAYS" :key="d" class="fw-semibold">{{ d }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="slot in SLOTS" :key="slot">
              <td
                class="text-muted align-middle text-center small fw-bold bg-body-light"
              >
                {{ slot }} - {{ SLOT_ENDS[slot] }}
              </td>
              <td
                ...
                :key="d"
                class="p-1 align-top"
                style="height: 120px; min-width: 160px"
              >
                <div
                  v-for="entry in getCell(d, slot)"
                  :key="entry.id"
                  class="rounded p-2 mb-1 border-start border-3 border-primary bg-primary-light text-primary position-relative"
                  style="font-size: 11px"
                >
                  <div class="fw-bold">{{ unitName(entry.unit_id) }}</div>
                  <div class="text-dark">
                    Class:
                    {{
                      store.classes.find((c) => c.id === entry.cohort_id)
                        ?.name || "N/A"
                    }}
                  </div>
                  <div class="small opacity-75">
                    Hall: {{ hallName(entry.hall_id) }}
                  </div>
                  <div class="small mt-1 text-muted">
                    Lec: {{ lecName(entry.lecturer_id) }}
                  </div>
                  <button
                    v-if="authStore.hasRole('ADMIN')"
                    @click="remove(entry)"
                    class="btn p-0 position-absolute"
                    style="top: 2px; right: 4px; opacity: 0.6"
                  >
  ...
  >
    <div class="row g-3">
      <div class="col-md-6">
        ...
        <select v-model="form.unit_id" class="form-select" required>
          ...
          <option v-for="u in store.units" :key="u.id" :value="u.id">
            {{ u.code }} - {{ u.name }}
          </option>
        </select>
      </div>
      ...
      <div class="col-md-6">
        ...
        <select v-model="form.hall_id" class="form-select">
          ...
          <option v-for="h in store.halls" :key="h.id" :value="h.id">
    ...
    <template #footer>
      ...
      <button
        ...
        @click="saveEntry"
        :disabled="saving || !form.unit_id || !form.cohort_id"
      >
        <span
          v-if="saving"
